...
class TestJiraParser:
    def test_can_parse_jira_api_format(self, parser: JiraParser, jira_fixture: Path) -> None:
        assert parser.can_parse(str(jira_fixture)) is True

    def test_can_parse_jira_list_format(self, parser: JiraParser, jira_multi_fixture: Path) -> None:
        assert parser.can_parse(str(jira_multi_fixture)) is True

    def test_cannot_parse_non_jira_json(self, parser: JiraParser, tmp_path: Path) -> None:
    ...
    def test_parse_api_format_extracts_issues(self, parser: JiraParser, jira_fixture: Path) -> None:
    ...
    def test_parse_extracts_issue_keys(self, parser: JiraParser, jira_fixture: Path) -> None:
        result = parser.parse(str(jira_fixture))
    ...
    def test_parse_extracts_comments(self, parser: JiraParser, jira_fixture: Path) -> None:
        result = parser.parse(str(jira_fixture))
        assert "John PM" in result.text
        assert "CSRF" in result.text

    def test_parse_extracts_relations(self, parser: JiraParser, jira_fixture: Path) -> None:
        result = parser.parse(str(jira_fixture))
        assert len(result.relations) > 0
        targets = [r["target"] for r in result.relations]
        assert "AUTH-103" in targets

    def test_parse_list_format(self, parser: JiraParser, jira_multi_fixture: Path) -> None:
    ...
    def test_parse_preserves_priority(self, parser: JiraParser, jira_fixture: Path) -> None:
        result = parser.parse(str(jira_fixture))
