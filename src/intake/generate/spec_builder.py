...
if TYPE_CHECKING:
    from intake.analyze.models import AcceptanceCheck, AnalysisResult
...
class SpecBuilder:
    ...
    def generate(
        ...
        for filename, template_name in SPEC_FILES.items():
            ...
            if filename == "acceptance.yaml":
                # Generate acceptance.yaml programmatically for guaranteed
                # valid YAML (LLM-generated strings may contain quotes and
                # special chars that break Jinja2 template rendering).
                content = self._build_acceptance_yaml(
    ...
    def _build_acceptance_yaml(
        self,
        checks: list[AcceptanceCheck],
    ) -> str:
        """Build acceptance.yaml programmatically for guaranteed valid YAML.

        Using yaml.dump() instead of a Jinja2 template ensures that strings
        containing quotes, special characters, or YAML metacharacters are
        properly escaped.

        Args:
            checks: Acceptance checks from the analysis result.

        Returns:
            Valid YAML string with a header comment.
        """
        header = (
            "# Acceptance checks — auto-generated by intake\n"
            "# Run with: intake verify <spec-dir>\n\n"
        )

        if not checks:
            return header + "checks: []\n"

        check_dicts: list[dict[str, object]] = []
        for check in checks:
            ...
            if check.type == "files_exist" and check.paths:
                entry["paths"] = check.paths
            if check.type in ("pattern_present", "pattern_absent") and check.glob:
                entry["glob"] = check.glob
            if check.type in ("pattern_present", "pattern_absent") and check.patterns:
                entry["patterns"] = check.patterns
            check_dicts.append(entry)

        data = {"checks": check_dicts}
        body = yaml.dump(
            data,
